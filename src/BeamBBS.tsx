...
const BeamBBS = () => {
  ...
  const [beam, setBeam] = useState<BeamState>({
    width: "230",
    depth: "380",
    mainFt: "60",
    exFt: "30",
  ...
  const totals = useMemo(() => {
    ...
    const steelWeight = (dia: number, nosStr: string, lengthM: number) => {
  ...
  return (
    <div style={{ padding: "20px", background: "#f5f8fa", minHeight: "100vh", fontFamily: "Arial" }}>

      <div style={{ maxWidth: "850px", margin: "auto", background: "#fff", padding: "30px", borderRadius: "15px", boxShadow: "0 5px 25px rgba(0,0,0,0.1)" }}>

        <h2 style={{ color: "#1a73e8", textAlign: "center", marginBottom: "30px" }}>
          Beam BBS Automation
        </h2>
...
const InputBox = ({label,val,set}:InputProps) => (
  <div style={{background:"#f1f3f4",padding:"12px",borderRadius:"8px"}}>
    ...
    <input
      ...
      style={{width:"100%",border:"none",background:"transparent",fontSize:"18px",textAlign:"center"}}
    />
  </div>
);
...
const RodRow = ({dia,val,set}:RodProps) => (
...
const TableRow = ({label,val,highlight=false}:RowProps) => (
