...
/**
 * UNIQ DESIGNS - FULL EXCEL LOGIC REWRITE
 * Logic strictly matches Excel "Steel Calculation Automation"
 * Target Results: 16mm ≈ 142.2 kg, 12mm ≈ 80.0 kg, 8mm ≈ 49.8 kg
 */
...
// EXACT BUNDLE DATA FROM EXCEL REFERENCE (IMAGE 39)
const REF_DATA: Record<number, { bundleWt: number; rodsInBundle: number; unitWt: number }> = {
  8:  { bundleWt: 47.4,  rodsInBundle: 10, unitWt: 0.400 },
  10: { bundleWt: 51.87, rodsInBundle: 7,  unitWt: 0.618 },
  12: { bundleWt: 53.35, rodsInBundle: 5,  unitWt: 0.890 },
  16: { bundleWt: 56.88, rodsInBundle: 3,  unitWt: 1.590 },
  20: { bundleWt: 59.26, rodsInBundle: 2,  unitWt: 2.470 },
  25: { bundleWt: 46.3,  rodsInBundle: 1,  unitWt: 3.860 }
};

const FEET_TO_METER = 3.281;
const ROD_LEN_M = 12.19; // EXACT MATCH FOR 40FT ROD (IMAGE 39, CELL I3)
const STIRRUP_CUT_FT = 3.5;
...
const UniqDesignsBBS: React.FC = () => {
  ...
  // --- THE CALCULATION ENGINE: 100% EXCEL MATCH ---
  const totals = useMemo(() => {
    ...
    beams.forEach(b => {
       ...
       * Required Bundles = (Total Meters / RodLength / RodsInBundle)
       * Weight = Required Bundles * Weight of one bundle
       */
      const getKg = (dia: number, nos: string, lengthM: number) => {
        const count = parseFloat(nos) || 0;
        if (count === 0 || !REF_DATA[dia]) return 0;
        const ref = REF_DATA[dia];
        const totalMeters = lengthM * count;
        const reqBundles = totalMeters / (ROD_LEN_M * ref.rodsInBundle);
        return reqBundles * ref.bundleWt;
      };

      // Summing Columns exactly like Excel AH through AM
      ...
      summary[b.stirrupDia] += getKg(b.stirrupDia, stirrupQty.toString(), STIRRUP_CUT_FT / FEET_TO_METER);
    });
    return summary;
  }, [beams]);
  ...
  const shareWA = () => {
    let msg = `*UNIQ DESIGNS - BBS SUMMARY*\n\n`;
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <div style={styles.btnRow}>
          <button onClick={shareWA} style={styles.waBtn}>WHATSAPP</button>
          <button onClick={() => window.print()} style={styles.pdfBtn}>PDF REPORT</button>
          <button onClick={() => setBeams([initialBeam(Date.now())])} style={styles.clearBtn}>CLEAR</button>
        </div>
      </header>
      ...
      {beams.map(b => (
        <div key={b.id} style={styles.card}>
          ...
          <div style={styles.row3}>
            <Field label="EX LEN" val={b.exFt} set={v => updateField(b.id, 'exFt', v)} />
...
const styles: Record<string, CSSProperties> = {
  ...
  pdfBtn: { background: '#6c757d', color: 'white', border: 'none', padding: '6px 12px', borderRadius: '6px', fontWeight: 'bold', fontSize: '10px' },
