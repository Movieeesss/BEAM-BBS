...
const DIVIDER = 3.281; // Feet to Meters (Excel Divider)
const ROD_LEN = 12;    // Rod Length in Meters
...
const BeamBBS = () => {
  ...
  // --- CALCULATION ENGINE: 100% EXCEL ROW MATCH ---
  const totals = useMemo(() => {
    ...
    beams.forEach(b => {
      ...
      const calcRowKg = (dia: number, nos: string, lenM: number) => {
        ...
        // ((Meters * Nos) / (RodsPerBundle * 12)) * BundleWeight
        return ((lenM * n) / (config.rods * ROD_LEN)) * config.weight;
      };

      // SUMMING EVERY INPUT INDEPENDENTLY (MATCHES COLUMNS AH - AM)
      // 16mm Calculations
