 ...
 * Calibrated for 100% Excel Match (38 Image Logic)
 */

// Reference data from Image 39 & 40
...
const ROD_LENGTH_METER = 12.19; // The critical 40ft standard from Excel
  ...
  const totals = useMemo(() => {
    ...
    beams.forEach(b => {
      ...
      const getKg = (dia, nos, lenM) => {
        ...
        // Excel Logic: Total Length / (RodLen * BundleCount) * BundleWeight
...
const Section = ({ title, d1, d2, bId, path, update, color }) => (
...
const RebarPair = ({ dia, nos, setDia, setNos }) => (
...
const styles = {
  ...
  header: { background: '#0d6efd', color: 'white', padding: '20px', borderRadius: '12px', textAlign: 'center', marginBottom: '20px', boxShadow: '0 4px 12px rgba(13, 110, 253, 0.3)' },
  title: { margin: 0, fontSize: '24px', letterSpacing: '1px' },
  ...
  removeBtn: { border: 'none', background: 'none', color: '#dc3545', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer' },
  ...
  select: { border: 'none', background: '#f1f5f9', padding: '5px 10px', fontSize: '14px', borderRight: '1px solid #cbd5e1', cursor: 'pointer' },
  nosInput: { width: '100%', border: 'none', textAlign: 'center', fontWeight: 'bold', fontSize: '16px', outline: 'none' },
  addBtn: { width: '100%', padding: '15px', borderRadius: '12px', background: 'white', border: '2px dashed #0d6efd', color: '#0d6efd', fontWeight: 'bold', cursor: 'pointer', transition: '0.2s' },
  footer: { position: 'fixed', bottom: 0, left: 0, right: 0, background: '#0d6efd', color: 'white', padding: '15px 20px', borderRadius: '25px 25px 0 0', boxShadow: '0 -5px 20px rgba(0,0,0,0.15)', zIndex: 100 },
